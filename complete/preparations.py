...
def main():
    ...
    if config.val:
        train_dataset = GetData(data[0:train_len, 0], data[0:train_len, 1:], t0=t0, to_predict=to_predict,
                                dec_len = dec_len, definite=config.definite)
        val_dataset = GetData(data[train_len:test_len, 0], data[train_len:test_len, 1:], t0=t0,
                              to_predict=to_predict,dec_len = dec_len, definite=config.definite)
        test_dataset = GetData(data[test_len:, 0], data[test_len:, 1:], t0=t0, to_predict=to_predict,dec_len = dec_len,
                               definite=config.definite)
        ...
        train_dataset = GetData(data[0:train_len, 0], data[0:train_len, 1:], t0=t0, to_predict=to_predict,
                                dec_len = dec_len, definite=config.definite)
        test_dataset = GetData(data[train_len:, 0], data[train_len:, 1:], t0=t0, to_predict=to_predict,
                               dec_len = dec_len, definite=config.definite)
        train_dl = DataLoader(train_dataset, batch_size=config.train_batch_size, shuffle=True)
        test_dl = DataLoader(test_dataset, batch_size=config.eval_batch_size, shuffle=False)
    ...
    b = [256]
    for _ in a:
        config.transformer['num_heads'] = _
        print('>>>>>>>>>>>>>>>>>>>>')
        print(f'Number of Heads: {_}')
        print('>>>>>>>>>>>>>>>>>>>>')
        Best_loss_rmse.append(_)
        Best_loss_mae.append(_)
        for d in b:
            config.embeddings['out_channels'] = d
            print('>>>>>>>>>>>>>>>>>>>>')
            print(f'Number of Dims: {d}')
            print('>>>>>>>>>>>>>>>>>>>>')
            Best_loss_rmse.append(d)
            Best_loss_mae.append(d)
            for exp in range(config.repeats):
                ...
                #optimizer = torch.optim.Adam(model.parameters(), lr=config.learning_rate,
                                             #weight_decay=config.weight_decay)  # Adam optimizer
                ...
                for epoch in tqdm(range(epochs)):
                    ...
                    if config.val:
                        ...
                        if val_best > val_mse:
                            ...
                            print('Current best validation MAE: {}'.format(val_mae))
                        ...
                        #trues.append(ground_truth.numpy())
                ...
                true_name = config.root + '\GT' + data_name + '.npy'
                pred_name = config.root + '\pred' + data_name + '.npy'
