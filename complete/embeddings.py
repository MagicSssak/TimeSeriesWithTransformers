...
class Embedding(torch.nn.Module):
    ...
    def __init__(self, d_model, conv_len, feature_dim, feature_weight =True, embedding_weight = False, s_dec = False):
        ...
        self.bn2 = torch.nn.BatchNorm2d(self.feature_dim+1)

        if feature_weight:
            self.input_embedding_f_Q = context_embedding(1, self.d_model, (self.feature_dim+1, self.conv_len), stride=(1, 1))
            self.input_embedding_f_K = context_embedding(1, self.d_model, (self.feature_dim+1, self.conv_len), stride=(1, 1))
            self.input_embedding_f_V = context_embedding(1, self.d_model, (self.feature_dim+1, 1), stride=(1, 1))
            l_dim = int(math.ceil(self.feature_dim / 1))
            self.feature_projection_Q = torch.nn.Linear(l_dim+1, 1)
            self.feature_projection_K = torch.nn.Linear(l_dim+1, 1)
            self.feature_projection_V = torch.nn.Linear(l_dim+1, 1)
        else:
    ...
    def forward(self, time, target, feature):
        ...
        if self.s_dec:
            ...
            if self.feature_weight:
                ...
                Qf_embedding = Qf_embedding.permute(0, 1, 3, 2).squeeze(-1)  # multi dim feature, B x C x L x F,
                ...
                Kf_embedding = Kf_embedding.permute(0, 1, 3, 2).squeeze(-1)
                ...
                Vf_embedding = Vf_embedding.permute(0, 1, 3, 2).squeeze(-1)
        ...
        if self.s_dec:
            Q = Qz_embedding + pe_encoder   ## add up directly
            K = Kz_embedding + pe_encoder
            V = Vz_embedding + pe_encoder
        else:

            Q = pe_encoder + Qz_embedding  ## add up directly
            K = pe_encoder + Kz_embedding
            V = pe_encoder + Vz_embedding
        #print(Q.shape)
