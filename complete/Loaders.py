...
class GetData(Dataset):
    ...
    def __init__(self, target, feature, t0=72, to_predict=24,dec_len = 24, transform=None, definite = True):
        ...
        for i in range(eff_len):
            ...
            if definite:
                ...
                time.append(torch.arange(1, enc_step+1).type(torch.float).unsqueeze(0))
                time_dec.append(torch.arange(1 + t0 - dec_len, enc_step + 1).type(torch.float).unsqueeze(0))
